...
export class ServicesRoutes {
  ...
  private setupRoutes(): void {
    ...
    this.setupStylistServiceRoutes(); // MÁS ESPECÍFICAS PRIMERO
    this.setupServiceRoutes();         // MENOS ESPECÍFICAS DESPUÉS
  }
  ...
  private setupStylistServiceRoutes(): void {
    // 1. PRIMERO: Rutas específicas con sufijos /active y /detailed
    this.router.get(
    ...
    // 2. DESPUÉS: Rutas genéricas con solo el ID
    this.router.get(
    ...
    // 3. Operaciones de modificación (POST, PUT, DELETE)
    this.router.post(
      '/services/stylists/:stylistId/services',
      this.authMiddleware.authenticate,
